...
% Last Modified by GUIDE v2.5 20-Mar-2014 14:24:30
...
function [x_ind, y_ind] = get_occ_index_from_xy(pose, handles)
% pass in [x,y] coordinates in camera view frame, return indices in
% occupancy grid.
[~, x_ind] = min(abs(handles.gr_x-pose(1)));
[~, y_ind] = min(abs(handles.gr_y-pose(2)));
...
%handle = imshow(cat(3,fliplr(handles.rgb(:,:,1)),fliplr(handles.rgb(:,:,2)),fliplr(handles.rgb(:,:,3))));
...
[goal_x, goal_y] = get_occ_index_from_xy(goal, handles);
...
[start_x, start_y] = get_occ_index_from_xy(start, handles);
